...
export type UseFormConfig<T extends FormikValues> = Omit<FormikConfig<T>, 'onSubmit'> &
  OwnConfig<T>;
...
export const useForm = <T extends FormikValues>({
  ...
  const field = useCallback(
    (name: DeepKeyof<T>, allowSubmit = false, showError = true) => ({
      onChangeText: formik.handleChange(name),
      onBlur: () => [formik.handleBlur(name), formik.setFieldTouched(name as string, true)],
      error: showError
  ...
  const submitProps = useMemo(
    () => ({
      ...
      disabled: formik.isSubmitting || (validateForm && !(formik.isValid && formik.dirty)),
    }),
    [formik, validateForm],
  );
